...
const AddServicePoint = () => {
    ...
    const validationSchema = Yup.object({
        ...
        monthly_billed: Yup.boolean(),
        credit_overdraft: Yup.boolean(),
        status: Yup.boolean(),
    ...
    const formik = useFormik({
        ...
        onSubmit: (data) => {
            ...
            if (params.id) {
                ...
                editServicePointMutation.mutate(formData)
            } else {

                addServicePointMutation.mutate(formData)
            }

        }
    })

    const addServicePointMutation = useMutation((data) => apiService.register(data), {
        onSuccess: (data) => {
    ...
    const editServicePointMutation = useMutation((data) => apiService.updateOneProfile(data, params.id), {
        onSuccess: (data) => {
            Toastify("success", data?.message)
            navigate("/members")
        },
            ...
            <Layout Header={(setVisible, visible) => <Header setVisible={setVisible} visible={visible} title="Member" />}>
                ...
                <div className="dist-container">
                    <div className="d-flex dist-heading">
                        ...
                        <p>{params.id ? "Edit Service Point" : "Add Servie Point"} </p>
                    </div>
                </div>
                ...
                <div className="add-btn" style={{ gap: "20px" }} >
                    ...
                    <button className="right-btn" type="button" onClick={handleSubmit} >{addServicePointMutation.isLoading || editServicePointMutation.isLoading ? "Please wait..." : params.id ? "Update" : "Add"}</button>
                    {
                        params.id &&
                ...
                <div className="dist-member-desc-container" >
                    ...
                    <div className="profile-control">
                        ...
                        <div className='profile-gallery'>
                            <h5 style={{ marginBottom: "20px", border: "none" }}>Logo</h5>

                            <div className='profile-images-area'>
                                {
                                    !values?.img && <div className='profile-image-area'>
                                ...
                                }
                                {
                                    ...
                                    <div className='profile-image-area'>
                                        <img src={values.img} alt='icon' width={50} height={50} />
                                    </div>

                                }
                            </div>
                        </div>
                        <button className='open-profile-media' onClick={() => ref.current.click()}>Upload</button>
                    </div>

                    <div className="credit-type">
                        <div className="checkbox">
                            <label>Credit Overdraft</label>
                            <input type="checkbox" name="credit_overdraft" onChange={(e) => {
                                setFieldValue("credit_overdraft", e.target.checked)
                                setFieldValue("monthly_billed", false)
                                setFieldValue("credit_based", false)
                            }} checked={values.credit_overdraft} />

                            {
                                errors.credit_overdraft &&
                                <div className="error-message">{errors.credit_overdraft}</div>

                            }
                        </div>
                        <div className="checkbox">
                            <label>Monthly Billed</label>
                            <input type="checkbox" v name="monthly_billed" onChange={(e) => {
                                setFieldValue("credit_overdraft", false)
                                setFieldValue("monthly_billed", e.target.checked)
                                setFieldValue("credit_based", false)
                            }} checked={values.monthly_billed} />

                            {
                                errors.monthly_billed &&
                                <div className="error-message">{errors.monthly_billed}</div>

                            }
                        </div>
                        <div className="checkbox">

                            <label>Create Based</label>
                            <input type="checkbox" checked={values.credit_based} name="credit_based" onChange={(e) => {
                                setFieldValue("credit_overdraft", false)
                                setFieldValue("monthly_billed", false)
                                setFieldValue("credit_based", e.target.checked)
                            }} />
                            {
                                errors.credit_based &&
                                <div className="error-message">{errors.credit_based}</div>
